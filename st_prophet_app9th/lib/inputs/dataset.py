...
from lib.utils.load import load_toy_dataset, load_custom_config, load_dataset
...
def input_dataset(
    ...
    if load_options["toy_dataset"]:
        ...
        df = load_toy_dataset(config["datasets"][dataset_name]["url"])
        load_options["dataset"] = dataset_name
        load_options["date_format"] = config["dataprep"]["date_format"]
        load_options["separator"] = ","
    else:
        file = st.file_uploader(
            "Upload a excel file", type=["xlsx", "xls"], help=readme["tooltips"]["dataset_upload"]
        )
